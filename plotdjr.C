...
void makeplot(const char *name, TTree *tree, TCut weight, const char *drawstring, const char *xlabel, int nbins, double xlow, double xhigh, int typeMC, const char * infile) {
  TFile collected (Form(infile))
  
  TH1D *hall = (TH1D*) collected.Get(TString::Format("hall_%s",name));
  TH1D *hmult0 = (TH1D*) collected.Get(TString::Format("hmult0_%s",name));
  TH1D *hmult1 = (TH1D*) collected.Get(TString::Format("hmult1_%s",name));
  TH1D *hmult2 = (TH1D*) collected.Get(TString::Format("hmult2_%s",name));
  TH1D *hmult3 = (TH1D*) collected.Get(TString::Format("hmult3_%s",name));
  TH1D *hmult4 = (TH1D*) collected.Get(TString::Format("hmult4_%s",name));
...
void plotdjr(const char *infile, const TString & outfile) {
