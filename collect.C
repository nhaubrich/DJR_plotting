...
void makeplot(const char *name, TTree *tree, TCut weight, const char *drawstring, const char *xlabel, int nbins, double xlow, double xhigh, int typeMC, const char * infile) {
  ...
  std::cout << "start drawing 1" << std::endl;
  

  TFile inputFile (Form(infile));
  TH1D *hall = (TH1D*) inputFile.Get(Form("hall_%s",name));
  TH1D *hmult0 = (TH1D*) inputFile.Get(Form("hmult0_%s",name));
  TH1D *hmult1 = (TH1D*) inputFile.Get(Form("hmult1_%s",name));
  TH1D *hmult2 = (TH1D*) inputFile.Get(Form("hmult2_%s",name));
  TH1D *hmult3 = (TH1D*) inputFile.Get(Form("hmult3_%s",name));
  TH1D *hmult4 = (TH1D*) inputFile.Get(Form("hmult4_%s",name));

  hall->SetDirectory(0);
  hmult0->SetDirectory(0);
  hmult1->SetDirectory(0);
  hmult2->SetDirectory(0);
  hmult3->SetDirectory(0);
  hmult4->SetDirectory(0);
...
void collect(const char * infile, const char * outfile) {
  ...
  pad[0]->cd();
  makeplot("djr0",tree,weight,"log10(GenEvent.DJRValues_[0])","DJR 0->1",nbins,djrmin,djrmax,typeMC,infile);
  pad[1]->cd();
  makeplot("djr1",tree,weight,"log10(GenEvent.DJRValues_[1])","DJR 1->2",nbins,djrmin,djrmax,typeMC,infile);
  pad[2]->cd();
  makeplot("djr2",tree,weight,"log10(GenEvent.DJRValues_[2])","DJR 2->3",nbins,djrmin,djrmax,typeMC,infile);
  pad[3]->cd();
  makeplot("djr3",tree,weight,"log10(GenEvent.DJRValues_[3])","DJR 3->4",nbins,djrmin,djrmax,typeMC,infile);
  pad[4]->cd();
  makeplot("djr4",tree,weight,"log10(GenEvent.DJRValues_[4])","DJR 4->5",nbins,djrmin,djrmax,typeMC,infile);
