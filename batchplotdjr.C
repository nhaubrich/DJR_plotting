...
void makeplot(const char *name, TTree *tree, TCut weight, const char *drawstring, const char *xlabel, int nbins, double xlow, double xhigh, int typeMC, const char *infile) {
  ...
  TFile outputFile (Form("hists_%s",infile),"UPDATE");
...
void batchplotdjr(const TString & infile) {
  ...
  TCut weight = "GenEvent.weight()*(((Sum$(patElectrons_slimmedElectrons__PAT.obj.pt()>20&& patElectrons_slimmedElectrons__PAT.obj.trackIso()<0.25)>2) || (Sum$(patMuons_slimmedMuons__PAT.obj.pt()>20&&patMuons_slimmedMuons__PAT.obj.trackIso()<0.25)>=2) ) && Sum$(patJets_slimmedJets__PAT.obj.pt()>30&&abs(patJets_slimmedJets__PAT.obj.eta())<2.5)>=2)";
